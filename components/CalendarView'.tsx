...
const arrowClass =
  "w-[70px] transition duration-[0.2s] scale-100 hover:scale-[1.01]";
...
function createDay(
  dayOnCalendar: number,
  monthOnCalendar: number,
  daysInMonth: number,
  currentDay: number,
  currentMonth: number,
  poolsOnDay: Array<Pool>,
) {
  ...
  return (
    <div
      className={
        ...
        " w-[13vw] md:w-[11.5vw] h-[168px] lg:h-[200px] xl:h-[225px] rounded mb-[8px] pb-[7px] flex flex-col justify-between"
      }
    >
      <div className="md:text-[16px] lg:text-[19px] xl:text-[22px] pt-[5px] pl-[7px]">
        {dayOnCalendar < 1 || dayOnCalendar > daysInMonth ? "" : dayOnCalendar}
      </div>
      <div className="flex flex-col items-center gap-[5px] mt-[6px] mb-[3px]">
        {poolsOnDay != undefined ? (
          poolsOnDay.slice(0, 2).map(function (pool) {
            return (
              <div className="bg-[#B4B4B4] w-[10.4vw] p-[2px_1px] g:p-[4px_3px] text-[10px] lg:text-[14px] xl:text-[17px] text-center rounded-slight pb-[6px]">
                <div className="text-left text-[12px] font-bold mt-[2px] ml-[3px] mb-[6px]">
                  {pool.time}
                </div>
        ...
        {poolsOnDay !== undefined && poolsOnDay.length > 2 ? (
          <div className="text-center mt-[1px] mb-[3px] text-[10px] lg:text-[14px] xl:text-[17px]">
            +{poolsOnDay.length - 2} more carpool
            {poolsOnDay.length - 2 == 2 ? "s" : ""}
          </div>
        ) : (
          <></>
        )}
...
export default function CalendarView(props: any) {
  ...
  const earliestDisplayYear =
    currentYear - Math.floor((currentMonth - monthsInPast) / 12);
  const latestsDisplayYear =
    currentYear + Math.floor((currentMonth + monthsInFuture) / 12);
  ...
  function nextMonth() {
    ...
    if (
      displayYear == latestsDisplayYear &&
      displayMonth == latestDisplayMonth
    ) {
  ...
  function prevMonth() {
    ...
    if (
      displayYear == earliestDisplayYear &&
      displayMonth == earliestDisplayMonth
    ) {
    ...
    <div className="flex flex-row items-center gap-[14px] w-fit m-auto">
      <div className={arrowClass} onClick={prevMonth}>
        <Image
          src={leftArrow}
          alt="<"
          className={atEarliestMonth ? "hidden" : ""}
        />
      </div>
      <div className={"w-[86vw] bg-card-bg rounded m-auto drop-shadow"}>
        <div className="text-center md:text-[70px] lg:text-[87px] pt-[8px]">
        ...
        <div className="grid-container grid items-center justify-items-center grid-cols-7 md:pt-[24px] lg:pt-[36px] pl-[10px] pr-[10px]">
          {daysOfWeek.map(function (day) {
            return (
              <div className="text-center md:text-[14px] lg:text-[18px] xl:text-[28px] mb-[4px]">
                {day}
              </div>
            );
          })}
          {Array.from(Array(42).keys()).map(function (i) {
            ...
            const dayOfWeekOfFirst = new Date(
              displayYear,
              displayMonth,
              1,
            ).getDay();
            const dayOnCalendar = i - dayOfWeekOfFirst + 1;
            const poolsOnDay = poolDateMap.hasOwnProperty(displayMonth)
              ? poolDateMap[displayMonth][dayOnCalendar]
              : undefined;
            return createDay(
              dayOnCalendar,
              displayMonth,
      ...
      <div className={arrowClass} onClick={nextMonth}>
        <Image
          src={rightArrow}
          alt="<"
          className={atLatestMonth ? "hidden" : ""}
        />
      </div>
    </div>
  );
