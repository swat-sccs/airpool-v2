...
export default function CarpoolCard(props: any) {
  ...
  const dotDiv = (
    <div
      className={`rounded-[50%] ${dotColor} ${props.displayType == "list" ? dotClassHoriztonal : dotClassBox}`}
    ></div>
  );
  ...
  const paymentMethodsDiv = (
    <div
      className={`flex flex-row ${props.displayType == "grid" ? "justify-center" : ""}`}
    >
  ...
  const vehicleTypeDiv = (
    <div className={styles["small-text"]}>{props.vehicleType}</div>
  );

  // Date-Time Div
  const dateTimeDiv = (
    <div
      className={`flex flex-col ${props.displayType == "list" ? "text-center" : "text-left"}`}
    >
      <div className={styles["regular-text"]}>{props.time}</div>
      <div className={styles["small-text"]}>{props.date}</div>
    </div>
  );
  ...
  if (props.displayType == "list") {
    ...
    const mainInfoDiv = (
      <div className="text-left">
        ...
        <div className="flex flex-row items-center">
          <div className="mr-[8px] md:mr-[12px] lg:mr-[25px]">
            {" "}
            {vehicleTypeDiv}{" "}
          </div>
          {paymentMethodsDiv}
    ...
    const layoutClass =
      "grid-container grid items-center grid-cols-12 sm:grid-cols-11 md:h-[100px] lg:h-[137px]";

    carpoolCard = (
      <div
        className={
          layoutClass +
          " bg-card-bg w-11/12 h-[81px] rounded drop-shadow pl-[5vw] pr-[4vw] md:pr-[6vw] lx:pr-[11vw] transition duration-[0.2s] scale-100 hover:scale-[1.01]"
        }
      >
        ...
        <div className="col-span-2 sm:col-span-3">{seatCountDiv}</div>
        <div className="col-span-4 sm:col-span-3">{dateTimeDiv}</div>
      </div>
    );
    ...
    const desinationDivWithDot = (
      <div className="flex flex-row items-center">
        <div className="mr-[4px] md:mr-[8px]">{dotDiv}</div>
        {desinationDiv}
      </div>
    );
    ...
    carpoolCard = (
      <SCCSBox
        contents={carpoolCardContents}
        extraClasses="w-[150px] h-[180px] md:w-[256px] md:h-[264px] lg:w-[357px] lg:h-[365px] transition duration-[0.2s] scale-100 hover:scale-[1.01]"
      />
    );
  } else {
