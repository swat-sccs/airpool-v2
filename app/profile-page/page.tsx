...
export default function Home() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const nameResult = await fetch("/api/create-user", {
      //fetch() = browser API for HTTP requests, gets from created api route
    ...
    <>
      <div className="text-left mt-[22px] ml-[20px]">
        <form onSubmit={handleSubmit}>
          {" "}
          {/*create form, calls function when form is submitted*/}
          <input
            value={name}
      ...
      <div className="flex gap-4">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div
            className={`flex flex-wrap items-center ${displayTypeIndex == 0 ? "flex-col" : "flex-row gap-[5px]"} mt-[5px]`}
          >
