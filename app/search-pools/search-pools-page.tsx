...
export default function Pools(props: { carpools: Carpool[] }) {
  ...
  const data = props.carpools.map(function (p) {
    const paymentTypes = [""];
    if (p.acceptsApplePay) {
      paymentTypes.push("Apple Pay");
    }
    if (p.acceptsCash) {
      paymentTypes.push("Cash");
    }
    if (p.acceptsVenmo) {
      paymentTypes.push("Venmo");
    }
    if (p.acceptsZelle) {
      paymentTypes.push("Zelle");
    }
    return (
      <CarpoolCard
        isDotBlue={true}
        destination={p.destination}
        vehicleType={p.transportationType.toString()}
        paymentMethods={paymentTypes}
        seatCount={p.availableSeats}
        time={p.meetingTime.toLocaleTimeString("en-US", { timeZone: "UTC" })}
        date={p.meetingTime.toDateString()}
  ...
  return (
    <>
      ...
      <div
        className={`flex flex-wrap items-center ${displayTypeIndex == 0 ? "flex-col" : "flex-row gap-[80px]"} mt-[40px]`}
      >
        {data}
      </div>
    </>
