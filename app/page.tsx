...
export default async function Home() {
  const mainButtonsClass =
    "w-[317px] md:w-[532px] transition duration-[0.2s] scale-100 hover:scale-[1.01]";

  const joinPoolButton = (
    <Link href="/search-pools">
      <button className={mainButtonsClass}>Join a Pool</button>
    </Link>
  );

  const createPoolButton = (
    <Link href="/create-pools">
      <button className={mainButtonsClass}>Create Pool</button>
    </Link>
  );
  ...
  return (
    <>
      <div className="text-center mt-[44px]">
        ...
        <div className="text-[24px] md:text-[30px] mt-[3px] ml-[208px] md:ml-[342px] mb-[10px]">
          by SCCS
        </div>
      </div>
      ...
      <div className="mt-[16px]">
        <SCCSBox
          extraClasses={mainButtonsClass}
          contents={[joinPoolButton, createPoolButton]}
        />
      </div>
